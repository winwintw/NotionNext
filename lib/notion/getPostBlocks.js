...
export async function getPage(id, from, slice) {
  ...
  if (pageBlock) {
    // console.log('[API<<--緩存]', `from:${from}`, cacheKey)
...
export async function getPageWithRetry(id, from, retryAttempts = 3) {
  if (retryAttempts && retryAttempts > 0) {
    console.log(
      ...
      `from:${from}`,
      `id:${id}`,
      retryAttempts < 3 ? `剩餘重試次數:${retryAttempts}` : ''
    )
    try {
      ...
      console.log('[API<<--響應]', `耗时:${end - start}ms - from:${from}`)
      return pageData
    } catch (e) {
      console.warn('[API<<--異常]:', e)
      await delay(1000)
      const cacheKey = 'page_block_' + id
    ...
    console.error('[請求失敗]:', `from:${from}`, `id:${id}`)
    return null
  }
}
